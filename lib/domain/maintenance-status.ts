...
export interface MaintenanceStatus { label: "Up to date" | "Attention soon" | "Action needed"; score: number; reasons: string[] }
export function calculateMaintenanceStatus(signals: MaintenanceSignals): MaintenanceStatus {
  ...
  const score = Math.max(0, 100 - deductions); const label = score >= 85 ? "Up to date" : score >= 60 ? "Attention soon" : "Action needed";
