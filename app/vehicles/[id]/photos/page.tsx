    ...
    <section className="gallery-layout" aria-labelledby="vehicle-gallery-heading">
      ...
      <div>
        ...
        {photos.length?<div className="vehicle-gallery-grid">{photos.map((photo,index)=><figure className={index===0?"gallery-featured":""} key={photo.id}>
          <Link aria-label={`View photo uploaded ${formatDate(photo.uploaded_at)} full size`} href={`/documents/${photo.id}`} target="_blank"><Image alt={`${vehicle.year} ${vehicle.make} ${vehicle.model} vehicle photo uploaded ${formatDate(photo.uploaded_at)}`} fill priority={index===0} sizes={index===0?"(max-width: 900px) 100vw, 48vw":"(max-width: 640px) 100vw, 28vw"} src={photo.url} unoptimized/></Link>
          <figcaption><div><strong>{index===0?"Latest photo":"Vehicle photo"}</strong><span>Uploaded {formatDate(photo.uploaded_at)}</span></div><div className="gallery-photo-actions"><Link aria-label={`Open photo uploaded ${formatDate(photo.uploaded_at)} full size`} href={`/documents/${photo.id}`} target="_blank"><ExternalLink size={16}/></Link><DocumentDeleteButton compact documentId={photo.id} vehicleId={id} fileName={photo.file_name}/></div></figcaption>
